...
fittingErrorPsa_two_pop = zeros([length(LIST),1]);  
forecastErrorPsa_two_pop = zeros([length(LIST),1]); 
fittingErrorAnd_two_pop = zeros([length(LIST),1]); 
forecastErrorAnd_two_pop = zeros([length(LIST),1]); 

fittingErrorPsa_three_pop = zeros([length(LIST),1]);  
forecastErrorPsa_three_pop = zeros([length(LIST),1]); 
fittingErrorAnd_three_pop = zeros([length(LIST),1]); 
forecastErrorAnd_three_pop = zeros([length(LIST),1]); 

fittingErrorPsa_portz = zeros([length(LIST),1]);  
forecastErrorPsa_portz = zeros([length(LIST),1]); 
        ...
        if (length(change) >= total_n) && b           
            ...
            switch model
                case 'two_pop'
                    ...
                    [errors,x0_new] =  objective(params,psa,androgen,t,change,x0,nFitting,model,f1,f2,f3,0);     % get new initial conditions, fitting errors, and plot fitting
                    errp = errors(1);
                    erra = errors(2);
                    
                    [errorsf] = future(params,psa,androgen,t,change,x0_new,nFitting,nForecast,model,f1,f2,f3,0);     % get forecasting errors and plot the forecasting
                    errpf = errorsf(1);
                    erraf = errorsf(2);
                ...
                case 'three_pop'
                    ...
                    [errors,x0_new] =  objective(params,psa,androgen,t,change,x0,nFitting,model,f1,f2,0,f4);     % get new initial conditions, fitting errors, and plot fitting
                    errp = errors(1);
                    erra = errors(2);
                    
                    [errorsf] = future(params,psa,androgen,t,change,x0_new,nFitting,nForecast,model,f1,f2,0,f4);     % get forecasting errors and plot the forecasting
                    errpf = errorsf(1);
                    erraf = errorsf(2);
...
function [errors,x0_new] = objective(params,psadata,and_data,tdata,change,x0,n,model,f1,f2,f3,f4) 
%{
...
switch model
    case 'two_pop'
        ...
        figure(f2)
        plot(tdata(1:length(AND)),AND,'m','linewidth',2);
        hold on;        
        
        figure(f3)
        plot(tdata(1:length(x1)),x1,'g','linewidth',2);
        hold on;
        plot(tdata(1:length(x2)),x2,'m','linewidth',2);
        hold on;     
    ...
    case 'three_pop'
        ...
        figure(f2)
        plot(tdata(1:length(AND)),AND,'g','linewidth',2);
        hold on;
        ...
        plot(tdata(1:length(x2)),x2,'m','linewidth',2);
        hold on;
        plot(tdata(1:length(x3)),x3,'b','linewidth',2);
        hold on;
...
function [errors]= future(params,psadata,and_data,tdata,change,x0,nFitting,nForecast,model,f1,f2,f3,f4) 
%{
...
switch model
    case 'two_pop'
        ...
        figure(f2);
        plot(tdata(change(nFitting+1):change(nFitting+1+nForecast)),AND,'m','linewidth',2);            % plot the forecasting model data
        hold on;
        
        figure(f3)
        plot(tdata(change(nFitting+1):change(nFitting+1+nForecast)),x1,'g','linewidth',2);
        hold on;
        plot(tdata(change(nFitting+1):change(nFitting+1+nForecast)),x2,'m','linewidth',2);
        hold on;
    ...
    case 'three_pop'
        ...
        figure(f2);
        plot(tdata(change(nFitting+1):change(nFitting+1+nForecast)),AND,'g','linewidth',2);            % plot the forecasting model data
        hold on;
        ...
        plot(tdata(change(nFitting+1):change(nFitting+1+nForecast)),x2,'m','linewidth',2);
        hold on;
        plot(tdata(change(nFitting+1):change(nFitting+1+nForecast)),x3,'b','linewidth',2);
        hold on;
